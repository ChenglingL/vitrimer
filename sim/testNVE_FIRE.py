...
def main():
    ...
    ap.add_argument("--duration_after_wait", type=int, default=10000000)
    ...
    for _ in range(n_stars_N1):
        ...
        pos, types, bonds = create_star(random_pos(box_length), arms,spacing=0.5, start_index=index_offset)
        all_pos.extend(pos)
    ...
    for i in range(len(all_pos)):
        all_pos[i] = wrap_into_box(all_pos[i],[box_length, box_length, box_length])
    ...
    # sim.operations.integrator.forces = [bond, lj, rev_cross]
    # while not (fire.converged):
    #     sim.run(100)
    
    integrator = hoomd.md.Integrator(dt=1e-3)
    integrator.forces = [bond, lj, rev_cross]
    ...
    def mkname(wait):
        return os.path.join(
                args.outdir,
                f"traj_T{tag(args.kT,8)}_rho{tag(args.rho,6)}_wait{wait}.gsd"
        )
