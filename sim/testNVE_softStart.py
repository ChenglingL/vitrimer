...
def main():
    ...
    ap.add_argument("--NVT_dt", type=float, default=1e-3)
    ap.add_argument("--NVT_timeSteps", type=float, default=1000000)
    ...
    sim.operations.integrator = hoomd.md.Integrator(dt=args.NVT_dt)
    ...
    #sim.run(100)
    print(f"Final box: {sim.state.box} \n")
    sim.operations.updaters.remove(box_resize)
    mttk2 = hoomd.md.methods.thermostats.MTTK(
        kT=args.kT,
        tau=0.1,
    )
    nvt2 = hoomd.md.methods.ConstantVolume(
        filter=hoomd.filter.All(),
        thermostat=mttk2
    )
    sim.operations.integrator.methods = [nvt2]
    ...
    print(f"NVT equilibrate \n")

    #sim.run(10000000)
    sim.run(args.NVT_timeSteps)
    
    print(f"NVE to record \n")
    ...
    sim.operations.integrator.dt = args.dt
    ...
    def mkname(wait):
        return os.path.join(
                args.outdir,
                f"traj_T{tag(args.kT,8)}_rho{tag(args.rho,6)}_NVTts{args.NVT_timeSteps}_NVTdt{args.NVT_dt}_wait{wait}.gsd"
        )
    ...
    f_table = open(os.path.join(
        args.outdir,
        f"energy_T{tag(args.kT,8)}_rho{tag(args.rho,6)}_NVTts{args.NVT_timeSteps}_NVTdt{args.NVT_dt}.log"
    ), "w")
    ...
    files = [{"filename": mkname(w), "wait": int(w), "end": None} for w in args.waits]
    base = base_log_steps(args.duration_after_wait, per_decade=20, include_end=True)
