...
def main():
    ...
    ap.add_argument("--k", type=float, default=37.037)
    ...
    bond.params['NN'] = dict(k=args.k, r0=1.5*0.9, epsilon=0.0, sigma=0.0,
                             delta=1.0)
    ...
    rev_cross.params[('A','B')] = {
        "sigma": 0.5, "n": 10, "epsilon": args.kT*100, "lambda3": 1}
    ...
    def mkname(wait):
        return os.path.join(
                args.outdir,
                f"traj_T{tag(args.kT,8)}_rho{tag(args.rho,6)}_ep{tag(args.kT*100,6)}_wait{wait}.gsd"
        )
